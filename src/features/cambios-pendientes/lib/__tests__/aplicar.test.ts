...
function mockService(opts: {
  ...
  const makeBuilder = (table: string) => {
    ...
    builder.maybeSingle = vi.fn(async () => ({ data: opts.maybeSingleData ?? null, error: null }))
    return builder
  }
  return {
...
describe('aplicarCambioPendiente', () => {
  ...
  it('datos_tutor_dni: fija dni_documento_path en la fila existente del vínculo', async () => {
    const updates: Array<{ table: string; patch: Record<string, unknown> }> = []
    const service = mockService({
      maybeSingleData: { id: 'dt1', dni_documento_path: null },
    ...
    expect(updates).toEqual([
      { table: 'datos_tutor', patch: { dni_documento_path: 'c/n/dni.pdf' } },
    ])
  })
