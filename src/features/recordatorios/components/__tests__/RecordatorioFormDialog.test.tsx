 ...
 * y PR #40). Por eso aquí solo verificamos que el componente monta y expone el
 * trigger; la lógica de creación está cubierta por:
...
describe('RecordatorioFormDialog (smoke)', () => {
  const destinos: RecordatorioDestinatarioInput[] = ['familia', 'direccion', 'personal']
  const ninos = [{ id: 'n1', nombre: 'Demo', apellidos: 'Uno' }]

  it('monta y muestra el botón "nuevo" sin reventar', () => {
    render(<RecordatorioFormDialog locale="es" destinos={destinos} ninos={ninos} />)
    expect(screen.getByTestId('recordatorios-nuevo')).toBeInTheDocument()
  })

  it('monta con destinos sin niño y lista de niños vacía', () => {
