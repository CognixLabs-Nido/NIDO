...
- [ ] **F8 — apretar `firmas_autorizacion` INSERT y la rama tutor de `autorizaciones_insert` a `es_tutor_legal_de`** (follow-up de F11-E, **NO** abordado ahí). Hoy ambas usan `es_tutor_de`, que **incluye `autorizado`**: `firmas_insert` (`es_tutor_de(nino_id) AND firmante_id = auth.uid() AND …`) y la rama B2 de `autorizaciones_insert` (recogida/medicación/imagen). **Firmar/instanciar una autorización es acto de guardián legal**, no del autorizado. Distinto del alta (F11-E): **requiere su propio análisis de casos legítimos** antes de apretar (¿algún flujo donde un `autorizado` deba firmar p. ej. su propia recogida? interacción con `puede_firmar_autorizaciones`, política de firmantes, recogida que nombra al autorizado como persona). Migración propia `CREATE OR REPLACE`/`DROP+CREATE POLICY`. _(Ola 1, F8 hardening)_
...
- [ ] **`createServiceClient` ([lib/supabase/server.ts](../../src/lib/supabase/server.ts)) NO bypasa RLS cuando hay sesión de usuario.** Está construido con `createServerClient` + cookies → adjunta el JWT del usuario logueado, que **prevalece sobre la service key** → actúa como ese usuario, no como `service_role`. (Distinto de `createServiceRoleClient` de [\_service-role.ts](../../src/features/auth/actions/_service-role.ts), cookie-less = service role real.) Origen: bug de la foto del niño (F11 P3b-2): el `UPDATE ninos.foto_url` afectaba 0 filas bajo la RLS del tutor; corregido en `ninos/[id]/foto/route.ts` migrando a `createServiceRoleClient`. **Acción:** auditar el resto de usos y migrar los que necesiten bypass real, **o** separar/renombrar los dos helpers (p. ej. `createUserScopedClient` vs `createServiceRoleClient`) para matar el footgun de raíz.
  - **PRIORIDAD ALTA — contexto SIN sesión de usuario (posible _under-delivery_ silenciosa: si el JWT no viaja, el cliente cae a `anon` y las lecturas cross-user devuelven 0 filas sin error):** `push/lib/audiencia` (×3), `push/lib/enviar-push`, `recordatorios/lib/audiencia`, `eventos/lib/audiencia`, `olvido/actions/purgar-vencidos`, `retencion/actions/barrer-retencion`. Estos calculan destinatarios / barren datos cross-user; si el bypass no es real, **entregan de menos o barren de menos sin avisar**. Verificar primero.
  - **Resto a revisar:** `informes/[id]/pdf`, `fotos/upload`, `export/[tipo]/[id]`, `export/me`, `agenda/lib/invitados` (×2), `agenda/queries/get-cita-detalle`, `agenda/actions/crear-cita`, `fotos/actions/gestionar-publicacion` (×2), `fotos/queries/get-publicaciones-aula`, `fotos/queries/get-publicaciones-familia`. Caso por caso: algunos corren con sesión admin (funcionan) o leen lo que el propio usuario ya ve (sin fuga).
