...
- [x] ~~**F12-B — Cuotas, recibos y remesas SEPA**~~ **CERRADA (B-0…B-8, 2026-07-01)** _(Ola 1, F12)._ Sucede a F11-G/H; consume el mandato SEPA (`mandatos_sepa.iban_cifrado` + `identificador_mandato`) de G-2/G-2bis. 11 tablas + 6 ENUMs + audit + RLS (B-0); motor de cierre atómico + recibos/líneas congeladas + becas/saldo (B-4); remesa SEPA pain.008 bajo demanda con IBAN cifrado (B-5); devoluciones/re-giros (B-6); vistas admin (pivote + CSV) y familia + aviso in-app derivado (B-7); ADR-0050 + gated RLS en CI (B-8). Ver `progress.md` (F12-B) y ADR-0050. **Follow-ups vivos abajo.**
  - ⚖️ **Dependencia RGPD con F11-B (posible abogado), a resolver antes del piloto con datos reales:** (a) **retención de recibos/remesas** (cuánto se conservan; los recibos van a `audit_log`); (b) el **XML pain.008** contiene IBANs en claro — decisión G1 = **no se almacena** (se genera bajo demanda y se descarga), pero el fichero descargado por la directora queda fuera del control de la app → debe constar en el **RAT** y en la política de tratamiento; (c) descifrado del IBAN (`get_mandatos_remesa`) admin-only server-side, nunca al cliente. _(Ola 1 — RGPD, F11-B — ABIERTO)_
  - [ ] **Secuencia SEPA FRST / primera-vez (diferida en B-5).** Hoy el generador pain.008 emite siempre `SeqTp=RCUR`. Un primer adeudo de un mandato debería ir como `FRST`. Requiere una **marca por mandato** ("ya se ha girado alguna vez") para decidir FRST vs RCUR. **Disparador:** si el banco lo exige o al primer giro real de un mandato nuevo. _(Ola 1 — F12-B, diferido)_
  - [ ] **`cobrado_manual` no distingue efectivo/transferencia (diferida en B-6).** Al marcar un recibo como cobrado*manual se conserva el `metodo`, pero no se registra por separado si el cobro manual fue en efectivo o por transferencia. Si contabilidad lo necesita, añadir el detalle. *(Ola 1 — F12-B, diferido)\_
  - ✅ **RESUELTO en B-6 — `devuelto` conserva la fecha de envío + `fecha_devolucion` nueva.** CHECK `recibos_envio_banco_fecha` reescrito (migración `20260701140000`): `devuelto` conserva `fecha_envio_banco` (las R-transactions referencian el envío original) y exige `fecha_devolucion`. Requisito 🔒 de B-6 cumplido.
  - ✅ **RESUELTO en B-4 — trigger de congelado del mes cerrado.** `congelar_si_mes_cerrado` (migración `20260630120000`, afinado en B-5 `20260701120000`) bloquea el parte y los recibos regulares + líneas de meses cerrados; el UPDATE de solo estado/fecha del ciclo de cobro pasa; esporádicos y re-giros exentos. Helper `mes_cerrado`. Decisión F cumplida.
  - ✅ **RESUELTO en B-4 — precio mensual vs diario.** `conceptos_cobro` gana `precio_mensual_centimos` + `precio_diario_centimos` + `servicio servicio_diario`. CHECK de coherencia por tipo. Esporádico reusa `precio_mensual_centimos`.
