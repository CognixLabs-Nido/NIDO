...
- [ ] **F12-B — Cuotas, recibos y remesas SEPA** _(EN CURSO; Ola 1, F12)._ Sucede a F11-G/H; consume el mandato SEPA (`mandatos_sepa.iban_cifrado` + `identificador_mandato`) de G-2/G-2bis. Subfases B-0…B-8, un PR por subfase. **B-0 (fundación, solo migración) abierto:** 11 tablas (`conceptos_cobro`, `tipos_beca`, `asignacion_cuota`, `becas`, `metodo_pago_familia`, `parte_servicio_diario`, `cierre_mensual`, `recibos`, `lineas_recibo`, `remesas`, `recibos_remesa`) + 6 ENUMs + audit + RLS. Decisiones A–K cerradas (2026-06-28). Pendiente B-1…B-8: catálogo, asignación/método/becas, parte diario (profes), motor de cierre, RPC `get_mandatos_remesa` + XML pain.008 bajo demanda, devoluciones, vistas + notificación, cierre (ADR + tests completos).
  ...
  - 🔒 **REQUISITO de B-4 (no opcional) — trigger de congelado del mes cerrado.** B-0 deja `recibos`/`lineas_recibo`/`parte_servicio_diario` editables por el admin/profe **sin** mirar `cierre_mensual`; la inmutabilidad de la decisión F **solo** está en el marcador `cierre_mensual`. B-4 DEBE añadir un trigger (patrón `bloquea_texto_tras_firma` de F8) que bloquee UPDATE/DELETE de `recibos`/`lineas_recibo` (y `parte_servicio_diario`) cuyo `(centro_id, anio, mes)` ya tenga fila en `cierre_mensual`. **Sin él la decisión F no se cumple.**
