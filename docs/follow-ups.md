...
- [ ] **Apretar el gate de las RPCs de escritura del tutor de 3a a `es_tutor_legal_de`** (F11-D/E). `actualizar_identidad_nino_tutor` y `set_info_medica_emergencia_cifrada_tutor` usan `es_tutor_de`, que **incluye `autorizado`** (cualquier `tipo_vinculo`). Escribir identidad/médica del niño es acto de guardián legal → debería ser `es_tutor_legal_de` (principal/secundario), como ya hace `marcar_matricula_lista` (P3c). RPC nueva con `CREATE OR REPLACE` (no editar las migraciones 3a aplicadas).
