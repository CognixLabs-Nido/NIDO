...
- [ ] **F12-B — Cuotas, recibos y remesas SEPA** _(EN CURSO; Ola 1, F12)._ Sucede a F11-G/H; consume el mandato SEPA (`mandatos_sepa.iban_cifrado` + `identificador_mandato`) de G-2/G-2bis. Subfases B-0…B-8, un PR por subfase. **B-0 (fundación, solo migración) abierto:** 11 tablas (`conceptos_cobro`, `tipos_beca`, `asignacion_cuota`, `becas`, `metodo_pago_familia`, `parte_servicio_diario`, `cierre_mensual`, `recibos`, `lineas_recibo`, `remesas`, `recibos_remesa`) + 6 ENUMs + audit + RLS. Decisiones A–K cerradas (2026-06-28). **B-0, B-1, B-2 y B-3 MERGEADOS; B-4 (motor de cierre + recibos) abierto.** Pendiente B-5…B-8: RPC `get_mandatos_remesa` + XML pain.008 bajo demanda, devoluciones (🔒 req. abajo), vistas + notificación, cierre (ADR + tests completos).
  ...
  - ✅ **RESUELTO en B-4 — trigger de congelado del mes cerrado.** `congelar_si_mes_cerrado` (migración `20260630120000`) bloquea INSERT/UPDATE/DELETE del parte y UPDATE/DELETE de recibos regulares + líneas cuyo `(centro,anio,mes)` tenga `cierre_mensual`; exentos esporádicos y devoluciones. Helper `mes_cerrado`. Decisión F cumplida.
  - 🔒 **REQUISITO de B-6 (no opcional) — `devuelto` conserva la fecha de envío + `fecha_devolucion` nueva.** Hoy el CHECK `recibos_envio_banco_fecha` obliga a `fecha_envio_banco IS NULL` salvo en `enviado_banco` → al pasar a `devuelto` se pierde cuándo se envió. Las R-transactions SEPA referencian el envío original. B-6 DEBE: relajar el CHECK para que `devuelto` **conserve** `fecha_envio_banco` y añadir columna `fecha_devolucion date` (cuándo se reflejó la devolución).
  - ✅ **RESUELTO en B-4 — precio mensual vs diario.** Se eligió la opción (a): `conceptos_cobro` gana `precio_mensual_centimos` + `precio_diario_centimos` (ambos nullable) + `servicio servicio_diario` (qué servicio del parte factura un diario; sin él el motor no sabe qué días contar). CHECK de coherencia por tipo. Esporádico reusa `precio_mensual_centimos`. Migración `20260630120000`; feature B-1 migrado.
