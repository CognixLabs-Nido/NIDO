...
## Funciones helper (crear en migración 001)

```sql
auth.es_admin(centro_id uuid)         → boolean
auth.es_profe_de_aula(aula_id uuid)   → boolean
auth.es_tutor_de(nino_id uuid)        → boolean
auth.tiene_permiso_sobre(nino_id uuid, permiso text) → boolean
auth.pertenece_a_centro(centro_id uuid) → boolean
```
