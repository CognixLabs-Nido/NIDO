...
- **D7 — Parentesco / tipo_vinculo / permisos.** El **tutor declara el parentesco** en el primer paso; la dirección fija `tipo_vinculo` en la invitación (**primer tutor = `tutor_legal_principal`**); `permisos` JSONB con los defaults del helper `permisosDefault` — **los mismos que crea hoy el camino admin** (consistencia entre ambos caminos). Son las **7 claves reales de `PERMISOS_KEYS`**, todas `true` para tutor*legal (principal/secundario) y `false` para autorizado: `puede_recoger`, `puede_ver_agenda`, `puede_ver_fotos`, `puede_ver_info_medica`, `puede_recibir_mensajes`, `puede_firmar_autorizaciones`, `puede_confirmar_eventos`. *(Las claves `puede_reportar_ausencias` y `puede_ver_datos_pedagogicos` NO están en `PERMISOS_KEYS`; el tutor ve lo pedagógico porque lo rellena él, no por permiso.)\_
