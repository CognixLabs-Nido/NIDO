...
- **Paquete RGPD — bloqueante ANTES de cargar el primer dato real.** Disparador: la primera familia/niño real en producción. Incluye:
  ...
  - 🔒 **Cifrado del IBAN del mandato SEPA (F11-G-2bis) — BLOQUEANTE DURO pre-piloto.** Ningún IBAN real puede entrar en BD antes de **mergear + aplicar** la migración `20260626120000_phase11g_2bis_cifrar_iban` (PR aparte de G-2/#151). El IBAN se cifra en reposo con pgcrypto (clave `sepa_encryption_key` en Vault, separada de la médica), espejo de `info_medica_emergencia` (ADR-0004); se descifra **solo** server-side por el proceso de remesas de dirección (Fase B, pain.008). **Prerrequisito de operador**: crear el secreto `sepa_encryption_key` en Vault **antes** de aplicar la migración (si no, la migración revierte). Mismo tier que el resto del paquete RGPD.
