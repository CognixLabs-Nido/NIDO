-- =============================================================================
-- B3 · Motor de recibos: la descripción de las líneas deja de embeber el nombre
-- -----------------------------------------------------------------------------
-- CREATE OR REPLACE de generar_recibos_mes(uuid,integer,integer). PARTE de la definición
-- VIVA del remoto (pg_get_functiondef) y cambia SOLO las cadenas de descripción: quita el
-- " · <nombre del niño>" que hasta ahora se pegaba a la etiqueta del concepto. De aquí en
-- adelante la descripción es el concepto LIMPIO (decisión de producto, Jose: opción (ii)).
--
-- Diff cuerpo VIVO vs NUEVO = EXACTAMENTE 8 líneas (una por INSERT que embebía el nombre);
-- ninguna toca lógica, importes, signos ni el flujo:
--   1) PASE 1 mensual   : r_asig.nombre || ' · ' || r_nino.nombre        -> r_asig.nombre
--   2) PASE 1 diario     : ... || ' · ' || r_nino.nombre || ' (N días)'   -> ... || ' (N días)'
--   3) PASE 2 becas fijas: 'Beca: ' || tb.nombre || ' · ' || r_nino.nombre -> 'Beca: ' || tb.nombre
--   4) PASE 2-bis comedor: 'Beca comedor · ' || r_nino.nombre             -> 'Beca comedor'
--   5) PASE 3 niño fijo   : r_desc.nombre || ' · ' || (SELECT nombre ...)  -> r_desc.nombre
--   6) PASE 3 niño %      : r_desc.nombre || ' · ' || (SELECT nombre ...)  -> r_desc.nombre
--   7) PASE 3 hermanos %  : r_desc.nombre || ' · ' || x.nombre            -> r_desc.nombre
--   8) PASE 3 hermanos fijo: r_desc.nombre || ' · ' || x.nombre           -> r_desc.nombre
--
-- Las líneas FAMILIARES (nino_id NULL: cargo de familia de PASE 1b, 'Saldo mes anterior')
-- ya no llevaban nombre → NO se tocan. Solo afecta a recibos NUEVOS: los ya existentes se
-- limpian al regenerar el mes (el motor borra el borrador y lo rehace); no hay migración
-- de datos. La vista interna (PanelMesRecibos) limpia defensivamente los viejos sin regenerar.
-- =============================================================================
