-- =============================================================================
-- D-6-2 · Motor de recibos: aplicar la beca comedor del mes (línea NEGATIVA)
-- -----------------------------------------------------------------------------
-- CREATE OR REPLACE de generar_recibos_mes(uuid,integer,integer). PARTE de la definición
-- VIVA del remoto (pg_get_functiondef de 20260801120000) y añade SOLO:
--   1) la variable `v_beca_com numeric;` en el DECLARE, y
--   2) el PASE 2-bis dentro del bucle de hijo, inmediatamente DESPUÉS del PASE 2 (becas
--      fijas de public.becas) y ANTES de cerrar ese LOOP.
-- El resto del cuerpo es IDÉNTICO (diff difflib: 12 líneas añadidas, 0 eliminadas).
--
-- PASE 2-bis: inserta la beca comedor variable del mes del niño (beca_comedor_mes, D-6-1)
-- como línea NEGATIVA independiente (concepto_id NULL, colgada del hijo), igual patrón que
-- las becas fijas del PASE 2. Mensual, ya cerrada: NO se prorratea ni se topa.
--
-- UNIDADES: beca_comedor_mes.importe está en EUROS (numeric(10,2)); lineas_recibo trabaja
-- en CÉNTIMOS (importe_centimos/precio_unitario_centimos = integer). Por eso ×100 y
-- round(...)::integer, negado. Verificado en el remoto: ambas columnas son integer en
-- céntimos y el PASE 2 (becas fijas, ya en céntimos) inserta -importe_centimos directo.
--
-- No se tocan PASE 1/1b/3/4, ni la firma, ni los GRANT, ni ninguna otra función.
    ...
    LOOP
      ...
      IF v_beca_com IS NOT NULL AND v_beca_com > 0 THEN
        INSERT INTO public.lineas_recibo
          (recibo_id, concepto_id, nino_id, descripcion, cantidad, precio_unitario_centimos, importe_centimos)
          VALUES (v_recibo, NULL, r_nino.id,
            left('Beca comedor · ' || r_nino.nombre, 200), 1,
