-- =============================================================================
-- RECIBOS · R-2 — `origen` en las líneas y el motor PRESERVA lo escrito a mano.
--
-- PROBLEMA. `generar_recibos_mes` era wipe+rebuild: borraba el BORRADOR ENTERO del mes
-- y lo recreaba con id nuevo. Cualquier línea escrita a mano moría por CASCADE, así que
-- no había forma de arreglar a mano un recibo sin que la siguiente regeneración lo
-- deshiciera.
--
-- SOLUCIÓN. Las líneas se marcan con `origen`; el reset vacía SOLO las automáticas y
-- REUTILIZA la carcasa del recibo (mismo id). Una línea origen='manual' el motor NO la
-- toca en ningún punto: ni la borra, ni la reescribe, ni la recalcula.
--
-- Cuatro implicaciones tratadas explícitamente, porque ninguna es gratis:
--   (a) `beca_comedor_desborde` se limpiaba por CASCADE al borrar el recibo. Tiene
--       UNIQUE(recibo_id): sin borrado explícito, la 2ª regeneración muere con 23505 y
--       aborta la generación entera. Se borra a mano en el reset.
--   (b) el descarte "si 0 líneas" cuenta el recibo entero, manuales incluidas → un
--       recibo cuyas únicas líneas sean manuales NO se autodestruye.
--   (c) la base del TOPE de la beca comedor suma el recibo entero, manuales incluidas
--       (decisión C1): un cargo a mano es cuota que la beca puede descontar.
--   (d) las bases del PASE 3 que DERIVAN un descuento (porcentual sobre concepto base, y
--       orden de hermanos) se acotan a origen='automatico' (decisión A): un ajuste a mano
--       no debe inflar un descuento ni mover el de hermanos a otro hijo.
--
-- Regla de coherencia entre (c) y (d): lo que DERIVA un descuento se calcula sobre el
-- catálogo; el TOPE de lo descontable se mide sobre lo que la familia paga de verdad.
--
-- La conversión "editar una línea automática la vuelve manual" (B1) es UI/action y va en
-- R-3; aquí solo entran el modelo y el motor.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. Columna `origen`. Aditiva: las líneas existentes quedan como 'automatico',
--    que es lo que son (todas las escribió el motor).
-- -----------------------------------------------------------------------------
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conrelid = 'public.lineas_recibo'::regclass AND conname = 'lineas_recibo_origen_check'
  ) THEN
    ALTER TABLE public.lineas_recibo
      ADD CONSTRAINT lineas_recibo_origen_check CHECK (origen IN ('automatico', 'manual'));
  END IF;
END $$;

COMMENT ON COLUMN public.lineas_recibo.origen IS
  'automatico = la escribió generar_recibos_mes y se rehace en cada regeneración; '
  'manual = la escribió una persona y el motor NUNCA la toca (R-2).';

-- -----------------------------------------------------------------------------
-- 2. El motor. Igual que la versión viva salvo: reset por líneas + reutilización
--    de la carcasa, guard de confirmados acotado, los dos filtros del PASE 3, y el
--    barrido final de borradores que el bucle ya no visita.
-- -----------------------------------------------------------------------------
